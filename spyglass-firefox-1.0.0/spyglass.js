(function () {
  ...
  container.innerHTML = finalCheckerHTML;
  ...
  function updateMiniRatioPill(minContrast, maxContrast, isRange, tweakTarget) {
    ...
    if (isRange) {
      miniRatioPill.innerHTML =
        `<span style="display:block; line-height:1.3; font-size:0.8rem;">${minContrast.toFixed(2)}:1</span>` +
        `<span style="display:block; line-height:1.3; font-size:0.8rem; border-top:1px solid ${pillColor}; margin-top:1px; padding-top:1px;">${maxContrast.toFixed(2)}:1</span>`;
    } else {
  ...
  function updateUI() {
    ...
    fgSwatch.value = fgHexWithAlpha.length > 7 ? fgHexWithAlpha.substring(0, 7) : fgHexWithAlpha;
    bgSwatch.value = bgHexWithAlpha.length > 7 ? bgHexWithAlpha.substring(0, 7) : bgHexWithAlpha;
    ...
    previewSummary.style.setProperty("--spyglass-summary-arrow-color", blendedFgHex);
    ...
    if (pixelAnalysisResult) {
      ...
      contrastRatioDisplay.innerHTML = `${contrastDisplay} <span style="font-size:0.6em; color:#6B7280;">(${typeLabel})</span>`;
    ...
    if (contrast < tweakTargetContrast) {
      ...
      if (newFgContrast >= tweakTargetContrast) {
        ...
        fgSuggestionSwatch.innerHTML = xIconSVG;
      ...
      if (newBgContrast >= tweakTargetContrast) {
        ...
        bgSuggestionSwatch.innerHTML = xIconSVG;
  ...
  overlayPickerBtn.addEventListener("click", async () => {
    ...
    const originalHTML = overlayPickerBtn.innerHTML;
    ...
    setTimeout(() => {
      ...
      overlayPickerBtn.innerHTML = originalHTML;
    }, 1000);
  });
  ...
  function smartTweakColor(colorInput, channel, amount) {
    ...
    const finalHex = currentContrast >= tweakTargetContrast 
        ? tweakedHex 
  ...
  function adjustLuminance(hexWithAlpha, targetContrast, otherHexWithAlpha, isAdjustingForeground) {
    ...
    const baseWhite = { r: 255, g: 255, b: 255, a: 1 };
    let startRgb = { r: colorRgba.r, g: colorRgba.g, b: colorRgba.b, a: colorRgba.a };
    ...
    let firstTryMixer = lumWhite > lumBlack ? { r: 255, g: 255, b: 255, a: 1 } : { r: 0, g: 0, b: 0, a: 1 };
    let secondTryMixer = lumWhite > lumBlack ? { r: 0, g: 0, b: 0, a: 1 } : { r: 255, g: 255, b: 255, a: 1 };
